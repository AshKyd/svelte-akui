...
- **`Masonry`**: A lightweight grid layout component that arranges items vertically based on their heights. Supports custom columns, grid gaps, and padding. Exposes a bindable `refreshLayout` method to manually trigger layout updates for dynamic content, an optional `allowResize` prop to conditionally enable or disable layout updates during container resizing transitions, an `animate` prop (with configurable `transitionDuration` and `transitionEasing`) to transition items as they slide into place, and `bind:items` + `itemSnippet` for data-driven rendering. Layout only — it does not implement dragging. To reorder or move cards, wrap items in `<Draggable>` and add `<DropTarget>`s; on drop, update the `items` array and Masonry animates every card to its new place.
...
- **`Wysiwyg`**: A rich-text WYSIWYG editor component powered by `@milkdown/crepe`. Loads its JavaScript bundle dynamically on-demand with a placeholder spinner during loading to enable code splitting. Fully integrated with light and dark mode. Supports custom paste hooks (`transformPastedHTML`, `transformPastedText`, `handlePaste`) for sanitisation and custom clipboard workflows. Exposes a `focus(collapseToStart?: boolean)` function to focus the editor. The "/" slash menu and block-drag-handle are opt-in via `slashMenu`: pass `true` for Crepe's defaults (headings, blockquotes, lists/checklists, images, code blocks, tables, math), or a `SlashMenuConfig` object to hide groups/items (set any entry to `null`) or extend the menu with your own commands via `buildMenu`.
...
- **`NotificationArea`**: A stacked live region of `InfoBox` notifications. Each item takes a `variant`, an optional `timeout` (auto-dismiss, minimum 5 s, `0` = sticky) with a countdown bar, a whole-body `onClick`, and an optional `actionLabel`/`onAction` button. Timers and bars pause while the list is hovered or focused; new items are announced through a matching `aria-live` region. `position` places it in a fixed corner or `inline`.
...
`Masonry` is layout only. To make cards draggable — to reorder within the grid, or move one onto something else — wrap each item in `<Draggable>` and give it (or a target elsewhere) a `<DropTarget>`. On a reorder drop, splice the `items` array; with `animate` on, every card glides to its new slot. See the "Reorder via Draggable" story.
